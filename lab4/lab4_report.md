...
**Первая часть:** На всех роутерах были настроены IP адреса на интерфейсах, OSPF и MPLS, iBGP, в том числе с route reflector кластером для 3 роутеров в центре схемы связи (HKI, LBN, LND), на 3 роутерах (SPB, NY, SVL) были настроены VRF, RD и RT на интерфейсах, через которые они соединены с компьютерами.
...
- Результаты некоторых пингов на роутерах
